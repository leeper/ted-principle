...
Wickham (2014) writes that "tidy datasets are all alike but every messy dataset is messy in its own way". The point is that most computational analyses require a particular kind of data structure - a tidy, rectangular dataset where rows are observations and columns are variables - which rarely if ever is immediately available. Much of a data analyst's efforts are therefore expended in the preprocessing stages of analysis, converting an *untidy* dataset into a *tidy* one. A tidy experimental dataset (TED) is a specific type of tidy dataset that has at a minimum two variables with values for each experimental unit:
 ...
 3. And, if not a constant, a numeric variable, specify each unit's weighting

The most basic design contains two columns: one factor variable representing "treatment" versus "control" experimental condition and one numeric variable representing a single outcome, with an implied equal weighting applied to all units. All other experimental datasets are elaborations upon this structure that reflect complexities of the design (e.g., factorial designs, multiple outcomes, missing data, covariates, etc.). The TED principle dictates that analysis of an experiment must begin with the creation of a TED. Table \ref{tab:ted} below shows a simple example of a TED for a two-condition experiment with a binary outcome:
...
Exerperimental analysis typically presumes either that the set of units being experimented on consitute a random sample of a population or, alternatively, that the units represent a meaningful population in and of themselves. In the former case, large-sample asymptotic properties can be used to justify parametric methods of analysis described above. In the latter, as well as the former, randomization inference (i.e., permutation testing) might also be used. Yet some experiments are performed on design-based with unequal unit selection probabilities and/or model-based adjustments to a design that require post-stratification corrections. In any of these cases, units are provided weights to adjust for nonrepresentativeness.

The TED principle implicitly assumes equal weighting (i.e., all units are weighted equally in the analysis). If this is not the case, a TED requires a `weight` variable.

 * sampling weights clustering
 * cluster identifiers and weights
...
These transformations convert the two variables into the current dataset into two new variables. In R:
...
Analysis of such designs proceeds as in a traditional factorial experiment, with the possible addition of unit fixed effects, clustering by respondent, and control for profile order.
